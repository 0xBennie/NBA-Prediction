...
# ── AI分析（可选） ─────────────────────────────────────────────────
def get_ai_analysis(game_info: str, injuries_info: str) -> str:
    """调用MiniMax AI生成简短分析"""
    if not MINIMAX_API_KEY:
        return ""
    try:
        system_prompt = """你是Polymarket NBA交易顾问。根据提供的数据，给出简洁的交易建议。
你只需要回复3部分，总共不超过200字：
1. 结论：推荐买入 / 观望 / 不推荐 + 一句话理由（必须提及伤病或近期状态作为依据）
2. 操作：入场价X¢，止盈X¢，止损X¢
3. 风险：影响判断的1-2个关键因素"""
            ...
            if clob_tokens and len(clob_tokens) >= 2:
                ...
                if clob_prices["valid"]:
                    ...
                    logger.info(
                        ...
                        f"away buy={clob_prices['away_buy']:.4f} sell={clob_prices['away_sell']:.4f} "
                        f"spread={clob_prices['away_spread']:.4f}"
                    )
...
class Scanner:
    ...
    def scan_futures(self):
        ...
        # 先评分所有期货，然后只推最高确定性的（按评分排序，最多推3个）
        scored = []
        ...
        for market, result in scored[:max_futures_push]:
            ...
            cid = market["condition_id"]
            logger.info(f"[Scanner] 期货推送 {market.get('team')} {market['type']} 评分:{result['score']}")

            # AI分析
            ai_text = ""
            if MINIMAX_API_KEY:
                futures_info = (
                    f"{market.get('team', '')} {market.get('type', '').upper()}\n"
                    f"市场价格: {market['price']:.1%}\n"
                    f"模型预测概率: {result.get('model_prob', 0):.1%}\n"
                    f"价值边际: +{result.get('value_edge', 0):.1%}\n"
                    f"结算天数: {market.get('days_to_resolution', 0)}"
                )
                ai_text = get_ai_analysis(futures_info, f"球队: {market.get('team', 'N/A')}")

                # AI过滤
                if ai_text:
                    ai_lower = ai_text.lower()
                    reject_kw = ["不推荐", "观望", "不入场", "不建议", "回避", "谨慎观望"]
                    if any(kw in ai_lower for kw in reject_kw):
                        logger.info(f"🚫 AI否决期货: {market.get('team')} {market['type']}")
                        continue
        ...
        try:
            ...
            self.db.insert("""
                ...
                WHERE NOT EXISTS (
                    ...
                    WHERE game_id=? AND scanned_at > datetime('now', '-50 minutes')
                )
            """, (
                game["game_id"],
                game["away_team"],
                ...
                game["game_id"],  # for WHERE NOT EXISTS check
            ))
        except Exception as e:
            logger.debug(f"[SignalLog] 记录失败: {e}")
    ...
    def _check_price_alerts(self):
        ...
        try:
            # 1. 价格低于目标价的机会
            alerts = self.watcher.check_opportunities()
            for a in alerts[:3]:
                if self.daily_push_count >= self.DAILY_LIMIT:
                    break
                ...
                # 伤病检查
                try:
                    inj = self.injury_checker.check_team(a["buy_team"])
                    if inj["has_star_out"]:
                        a["model_prob"] -= inj["injury_penalty"]
                        a["value_edge"] = a["model_prob"] - a["current_price"]
                        if a["model_prob"] < 0.55 or a["value_edge"] < 0.03:
                            logger.info(f"🚫 低价机会被伤病否决: {a['buy_team']} {inj['details']}")
                            self.watcher.mark_alerted(a["game_id"])
                            continue
                except Exception:
                    pass
                ...
                msg = (
                    ...
                    f"🔮 模型预测: <b>{a['buy_team']}胜 {a['model_prob']:.1%}</b>\n"
                    f"💰 当前价格: <b>{a['current_price']:.1%}</b> ← 低于目标{a['target_price']:.1%}\n"
                    f"📉 历史: 最低{a['lowest_price']:.1%} / 最高{a['highest_price']:.1%}\n"
                    f"📈 价值边际: <b>+{a['value_edge']:.1%}</b>\n"
                    f"⚖️ 赔率: 赢<b>+{roi:.0%}</b> / 输-100%\n"
                )
                if slug:
                    msg += f"\n🔗 <a href=\"https://polymarket.com/event/{slug}\">Polymarket</a>\n"
                msg += f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M')}"
                ...
                if self.trader and token_id:
                    bet_size = self.trader.calculate_bet_size(0.03)  # 3% Kelly
                    if bet_size > 0:
                        self._send_trade_buttons(
                            msg, token_id, a["current_price"],
                            bet_size, a["game_id"], a["buy_team"]
                        )
                        self.watcher.mark_alerted(a["game_id"])
                        self.daily_push_count += 1
                        logger.info(
                            f"🔔 低价机会: {a['away_team']}@{a['home_team']} "
                            f"买{a['buy_team']} {a['current_price']:.0%} (目标{a['target_price']:.0%})"
                        )
                        continue
                ...
                logger.info(
                    f"🔔 低价机会: {a['away_team']}@{a['home_team']} "
                    f"买{a['buy_team']} {a['current_price']:.0%}"
                )
            ...
            logger.warning(f"[Watcher] 价格检查失败: {e}")
